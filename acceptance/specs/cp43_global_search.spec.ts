...
test.describe('global search', () => {
  test('finds matching clients and projects', { tag: '@functionality' }, async ({ page }) => {
    ...
    await expect(page.getByTestId('search-projects').getByTestId(/^project-row-/)).toHaveCount(1);
    await expect(page.getByTestId('search-projects').getByTestId('project-row-1').getByTestId('project-name')).toHaveText('Acme Website');
  });
});
