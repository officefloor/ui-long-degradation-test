...
test.describe('client detail', () => {
  test("lists the client's active projects by default", { tag: '@core' }, async ({ page }) => {
    ...
    const owned = page.getByTestId('client-jobs-table');
    await expect(owned.getByTestId(/^job-row-/)).toHaveCount(1);
    await expect(owned.getByTestId('job-row-1').getByTestId('job-name')).toHaveText('Website Rebuild');
    await expect(owned.getByTestId('job-row-2')).toHaveCount(0);
    await expect(owned.getByTestId('job-row-4')).toHaveCount(0);
  });
});
