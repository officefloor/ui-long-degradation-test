...
test.describe('project budget', () => {
  test('shows budget, invoiced and remaining', { tag: '@functionality' }, async ({ page }) => {
    ...
    await expect(page.getByTestId('job-budget')).toHaveText('$1,000.00');
    await expect(page.getByTestId('job-invoiced')).toHaveText('$500.00');
    await expect(page.getByTestId('job-remaining')).toHaveText('$500.00');
  });
});
