...
test.describe('client active projects by default', () => {
  test('shows active projects, with a toggle for the rest', { tag: '@functionality' }, async ({ page }) => {
    ...
    const table = page.getByTestId('client-jobs-table');
    await expect(table.getByTestId(/^job-row-/)).toHaveCount(1);
    await expect(table.getByTestId('job-row-1').getByTestId('job-name')).toHaveText('Website Rebuild');

    await page.getByTestId('client-jobs-show-all').click();
    await expect(table.getByTestId(/^job-row-/)).toHaveCount(3);
  });
});
